...
\`spec_values_at()\` extracts spectral values at specified wavenumber
positions. The function finds the closest wavenumber matches and returns
the corresponding spectral intensities for each sample column.
...
- method:
  ...
  Character string specifying the extraction method. Options: "closest"
  (default), "exact", "interpolate". See Details.

- tolerance:

  Numeric value specifying the maximum allowed difference for "closest"
  method. Default: Inf (no limit).

- format:

  Character string specifying output format. Options: "wide" (default),
  "long". See Details.
...
Extraction methods: - "closest": Find the closest wavenumber match
(default) - "exact": Only return exact matches (NA for non-matches) -
"interpolate": Use linear interpolation between adjacent points

Output formats: - "wide": Each wavenumber becomes a row, samples as
columns - "long": Tidy format with wavenumber, sample, and value columns
