...
The goal of **tidyspec** is to provide a friendly pipeline for spectroscoy analysis using the tidy data philosophy.
...
The `tidyspec` package was design to enable the data analysis of spectral data (as IR, Raman, NMR) with the tidy-data format. There are 6 families of functions in `tidyspec`, all starting with `spec_`:
...
- **Normalize**: Normalize the data to range 0-1 (`spec_norm_01`) or normalize to have a standart deviation of one (`spec_norm_var`).  
- **Baseline correction**: Correct the baseline using the the *rolling ball* algorithm (`spec_blc_rollingBall`).  
